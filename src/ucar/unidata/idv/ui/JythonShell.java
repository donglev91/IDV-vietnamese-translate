...
public class JythonShell extends InteractiveShell {
    ...
    public void showProcedurePopup(JTextComponent cmdFld) {
        ...
        if (history.size() > 0) {
            ...
            items.add(GuiUtils.makeMenu("Lịch Sử", historyItems));
    ...
    protected JMenuBar doMakeMenuBar() {
        ...
        menuBar.add(GuiUtils.makeMenu("Tệp", items));
        ...
        menuBar.add(GuiUtils.makeMenu("Chỉnh Sửa", items));

        items = new ArrayList();
        items.add(GuiUtils.makeMenuItem("Trợ Giúp", this, "showHelp"));
        menuBar.add(GuiUtils.makeMenu("Trợ Giúp", items));
        return menuBar;
    }
